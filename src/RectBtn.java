...
public class RectBtn extends AbstractBtn {
	...
	public void display() {
		...
		float fontSize = this.getWidth()/this.getName().length();
		...
		tmpParent.fill(255);
		tmpParent.text(this.getName(), this.getX() +(this.getWidth() - this.getName().length() * fontSize/2)/2  , this.getY() + this.getHeight()*2/3);
	}
